...
As an example, suppose that you're a regular meat eater, and want to reduce the GHG footprint of your diet. As is the case for many people, you realize that consuming beef accounts for most of that footprint, and so decide to reduce these beef-related GHG-emissions by a factor of 10 (therefore, a reduction of -90%, or equivalently -100 cB).
...
While environmental questions are often framed in such binary terms, the above figure shows us that there is in fact a continuum of possible responses, and gives us more precise answers than a vague _"well, we should to both"_.
